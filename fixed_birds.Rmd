...
for (i in seq_along(knb_files)) {
  assign(knb_files[i] %>%
           str_remove("52_pp52_") %>%
           str_remove(".csv"), 
         read_csv(here("knb-lter-cap_256_9", knb_files[i])))
}
...
```{r, include = FALSE}
birds_df <- left_join(taxalist_1, birds_1, by = "species_id")
```
...
```{r}
graph <- ggplot(data = selected_parks, aes(y = common_name, x = total_count)) +
  geom_col() +
...
```{r}
ggsave(plot = graph, here("bird_counts.jpg"))
```
